...
class BlackAndWhite(src: Image) extends Image(src.height, src.width):
  ...
  def grayscale(input: Pixel) =
    val grayValue = (lumaR * red(input) + lumaG * green(input) + lumaB * blue(input)).toInt
    argb(alpha(input), grayValue, grayValue, grayValue)
...
class RedSplash(src: Image) extends BlackAndWhite(src):
  ...
  override def apply(x: Int, y: Int): Pixel = 
    val pix = src(x, y)
    if (!isRedEnough(pix)) grayscale(pix)
    else pix
...
class SimpleBlur(src: Image) extends Image(src.height, src.width):
  ...
  def apply(x: Int, y: Int): Pixel =
    def isValidPixel(px: Int, py: Int): Boolean = px >= 0 && px < src.width && py >= 0 && py < src.height
    ...
    val neighborhoodSum = (for {
      i <- -radius to radius
      j <- -radius to radius
      if (isValidPixel(x + i, y + j))
    } yield src(x + i, y + j))

    val (totalAlpha, totalRed, totalGreen, totalBlue) = neighborhoodSum.foldLeft((0, 0, 0, 0)) { (acc, pixel) =>
    (acc._1 + alpha(pixel), acc._2 + red(pixel), acc._3 + green(pixel), acc._4 + blue(pixel))}
...
class Convolution(src: Image, kernel: Kernel) extends Image(src.height, src.width):
  def apply(x: Int, y: Int): Pixel =
    var (totalAlpha, totalRed, totalGreen, totalBlue) = (0, 0, 0, 0)

    for (dx <- 0 until kernel.width; dy <- 0 until kernel.height) 
      val srcX = x - (kernel.width / 2) + dx
      val srcY = y - (kernel.height / 2) + dy

      if (srcX >= 0 && srcX < width && srcY >= 0 && srcY < height) 
        totalAlpha += (kernel(dx, dy) * alpha(src(srcX, srcY))).toInt
        totalRed += (kernel(dx, dy) * red(src(srcX, srcY))).toInt
        totalGreen += (kernel(dx, dy) * green(src(srcX, srcY))).toInt
        totalBlue += (kernel(dx, dy) * blue(src(srcX, srcY))).toInt
    
    argb(totalAlpha, totalRed, totalGreen, totalBlue)
